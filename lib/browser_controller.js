...
async function startVNCStack() {
    ...
    // Removed -noxdamage so x11vnc uses native X11 extensions instead of 100% CPU polling.
    const vnc = spawn('x11vnc', ['-display', ':99', '-forever', '-shared', '-nopw', '-rfbport', '5900', '-noshm'], { shell: true });
...
export async function launchInteractiveBrowser(email, db) {
    ...
    // Launch Chrome in "headful" mode on the virtual display using Playwright's bundled Chromium
