...
let vncProcesses = []; // Track background VNC processes to clean up later

async function startVNCStack() {
    console.log('[VNC] Starting Xvfb + x11vnc + websockify on-demand...');
    ...
    // Give Xvfb time to initialize asynchronously so we don't block Node event loop (prevents HF 503)
